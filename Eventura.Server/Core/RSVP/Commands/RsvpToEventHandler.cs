...
public class RsvpToEventHandler : IRequestHandler<RsvpToEventCommand, Unit>
{
    ...
    public async Task<Unit> Handle(RsvpToEventCommand c, CancellationToken ct)
    {
        ...
        if (ev.IsBlocked) throw new InvalidOperationException("Event is blocked.");

        var exists = await _db.Rsvps.AnyAsync(r => r.EventId == c.EventId && r.UserId == c.UserId, ct);
        if (exists) return Unit.Value; // idempotent

        _db.Rsvps.Add(new Rsvp
        {
            EventId = c.EventId,
            UserId = c.UserId,
